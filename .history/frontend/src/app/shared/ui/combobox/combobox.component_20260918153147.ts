    ...
    <div class="w-full flex flex-col gap-1.5" (keydown)="onKeydown($event)" #container>
      ...
      <div class="relative w-full">
        ...
        <div 
          *ngIf="isOpen"
          [id]="'listbox-' + labelId"
          class="absolute z-50 mt-1 w-full rounded-md bg-white shadow-md ring-1 ring-black ring-opacity-5 focus:outline-none animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-60 max-sm:fixed max-sm:inset-x-0 max-sm:bottom-0 max-sm:mt-0 max-sm:max-h-[75vh] max-sm:rounded-b-none"
          role="listbox"
          ...
          <!-- Options list -->
          <div class="sr-only" aria-live="polite">{{ filteredOptions.length }} result{{ filteredOptions.length === 1 ? '' : 's' }}</div>
